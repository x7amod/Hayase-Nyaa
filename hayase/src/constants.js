...
export const ANY_RESOLUTION_P_RE = /\b\d{3,4}p\b/i
...
export const RANGE_FRAGMENT_RE = /\b0*\d{1,4}\s*[-~]\s*0*\d{1,4}\b/
export const ROMAN_RE = /\b(XXX?IX|XXX?IV|XXX?V?I{0,3}|XXI{0,3}|XIV|XIX|XL|XLIX|X{1,3}|IV|V|VI{0,3}|IX|I{1,3})\b/g
export const CLASSIFY_RANGE_RE = /\b0*(\d{1,4})(?:[-~]|to)0*(\d{1,4})(?=\D|$)/gi
...
export const SEASON_S_RE = /\b[Ss](?:eason)?\s*0*(\d{1,2})\b/
export const SEASON_ORDINAL_RE = /\b(\d{1,2})(?:st|nd|rd|th)\s+[Ss]eason\b/
export const TRAILING_ORDINAL_RE = /\b(\d{1,2})(?:st|nd|rd|th)\b\s*$/
export const SIZE_MULTIPLIERS = { KB: 1e3, MB: 1e6, GB: 1e9, TB: 1e12, KIB: 1024, MIB: 1048576, GIB: 1073741824, TIB: 1099511627776 }
